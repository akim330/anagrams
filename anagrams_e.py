...
class banana(object):
    def __init__(self):
        ...
        self.time_flip_request = pygame.time.get_ticks()
        self.time_to_take = pygame.time.get_ticks()
        self.time_confirmed_take = pygame.time.get_ticks()
        self.take_key = 0
    ...
    def send_data(self):
        ...
        data = str(self.net.id) + "|" + str(self.seed) + "|" + str(self.current) + "|" + str(self.playerwords) + "|" + str(self.playerwords_list) + "|" + str(self.player2words) + "|" + str(self.player2words_list) + "|" + str(self.last_update) + "|" + str(self.used_tiles) + "|" + str(self.new_word_i) + "|" + self.taken_word + "|" + str(self.flip_waiting) + "|" + str(self.time_to_take) + "|" + str(self.take_confirmed) + "|" + str(self.take_key)
    ...
    def parse_data(data):
        try:
            ...
            time_to_take_recv = try_parsing_date(split[12])
            take_confirmed_recv = ast.literal_eval(split[13])
            take_key_recv = ast.literal_eval(split[14])

            return net_id, seed_recv, current, player2words, player2words_list, playerwords, playerwords_list, last_update, used_tiles_recv, new_word_i_recv, taken_word_recv, flip_waiting_recv, time_to_take_recv, take_confirmed_recv, take_key_recv
        except:
            return -1, 0, None, {}, [], None, None, datetime.datetime(1, 1, 1, 0, 0), [], -1, '', False, 0, False
    ...
    def take(self, candidate):
        ...
        if event_type == 'steal' or event_type == 'middle':
            if self.time_flip > self.time_confirmed_take:
                self.time_to_take = self.take_start_time - self.flip_time
            else:
                self.time_to_take = self.take_start_time - self.time_confirmed_take

            self.take_confirmed = False

            self.take_key = self.take_start_time
    ...
    def printstatus(self):
        ...
        if self.mode != 'solo':
            ...
            if time.time() - self.last_type > 0.5:
                net_id_recv, seed_recv, self.player2current, player2words_recv, player2words_list_recv, playerwords_recv, playerwords_list_recv, self.player2_last_update, used_tiles_recv, new_word_i_recv, taken_word_recv, flip_waiting_recv, time_to_take_recv, take_confirmed_recv, take_key_recv = self.parse_data(self.send_data())
        ...
        if self.player2_last_update > self.last_update:
            ...
            print(f"Second check: {any(x in self.used_tiles for x in used_tiles_recv)}")
...
def main():
    ...
    while True: # main game loop
        ...
        if game.flip_waiting:
            if pygame.time.get_ticks() - game.time_flip_request > flip_delay:
        ...
        if not game.take_confirmed:
            game.status = 'Confirming take...'
            game.printstatus()
            ...
            if game.frozen:
                ...
                if event.key == K_BACKSPACE:
                    ...
                    if game.guess == '':
                        ...
                        game.time_flip_request = pygame.time.get_ticks()
